...
<?php
	...
	function display_report_data($main_qry, $report_name, $report_header, $rep_parameter) {  

		if ($report_name == "report_show_unassign_lead"){		

			$sorturl = htmlentities($_SERVER['PHP_SELF']. "?reprun=yes&sel_status=" . $rep_parameter);

		}else{

			$sorturl = htmlentities($_SERVER['PHP_SELF']. "?reprun=yes");
		...
		<table class="new_search_tbl" cellpadding="0" cellspacing="0" width="100%">
			...
			//$main_qry will come from calling report

			//$main_qry = "Select companyInfo.ID, companyInfo.company, companyInfo.city, companyInfo.state, employees.initials AS EI from companyInfo 

			//LEFT OUTER JOIN employees ON companyInfo.assignedto = employees.employeeID  where ". $str_filter ;
			...
			?>
				...
				<tr align="center">
					...
					<td colspan="14" bgcolor="#FFCCCC"><b><?php echo $report_header . " - Total Records: " . $srno; ?>
...

			{
				...
				//echo $comp_list_arr . "<br>";

				$salescomp = "";

				$sql = "SELECT parent_child,parent_comp_id, loopid, haveNeed, contact, on_hold, ucbzw_flg FROM companyInfo Where ID  ='".$comp_list_arr."'";
				db_b2b();
...

				{
					...
					?>
						...
						<tr valign="middle" id="tbl_div<?php echo $data["I"]?>">
							...
							<td width="21%" bgcolor="<? echo $bg_color;?>">

								<!-- Popup display start -->

								<div id="showCompDt<? echo $data['I']; ?>" style="display:none;">
										...
										<table width="500px" >
											...
											<tr>
												...
												<td bgcolor="<?php echo $bg_color;?>">
													...
													<a href="https://loops.usedcardboardboxes.com/viewCompany.php?ID=<?=$cmpDt["comp1Id"]?>" target='_blank'><? echo $nickname1; ?></a>
								...
								<a href="javascript:void(0)" id="compid<?php echo $data['I']; ?>" onclick="openCompPopup(<?php echo $data['I']; ?>)"  ><? echo $new_nickname; ?></a>
							...
							<td bgcolor="<?php echo $bg_color;?>" align="center">
								...
								<a href="#" id="com_contact<?php echo $data["I"]; ?>" onclick="showcontact_details(<?php echo $data["I"]; ?>, '<?php echo $searchcrit; ?>'); return false;"><?php echo $font_str_tag;?><img src="images/contact_plus.png" width='13' height='13' /><?php echo $font_str_tag2;?></a><?php echo highlightSearchKeywords($data["C"],$searchcrit); ?></td>
							...
							<?php if ($data["UZWND"]!="" && $data["UZWND"] != "1969-12-31") {?>
								...
								<td align="center" bgcolor="<?php echo $bg_color;?>">
									...
									<?php if ($data["UZWND"]!="") {?>
										...
										<div style="<?php if ($data["ND"] == date('Y-m-d')) { ?> background:#00FF00; <?php } elseif ($data["ND"] < date('Y-m-d') && $data["ND"] != "") { ?> background:#FF0000; <? } else { ?> background:<?php echo $bgcolor_str;?> <? } ?>" >
