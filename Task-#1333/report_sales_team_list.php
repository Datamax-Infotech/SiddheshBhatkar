...
<html>
   <head>
      <title>Call List Report - B2B Sales</title>
   ...
   <body>
      ...
      <div class="main_data_css">
         ...
         <?php
            ...
            $sql_time = "Select * from sales_team_queue_2 limit 1";
         ...
         <div class="dashboard_heading" style="float: left;">
            <div style="float: left;">
               Call List Report - B2B Sales 
               <div class="tooltip"><i class="fa fa-info-circle" aria-hidden="true"></i>
                  <span class="tooltiptext">This report shows the user all B2B sales accounts in the system which meet the conditions listed. These conditions are designed to generate a focused report of accounts which should have a higher probability to yield sales. This list is refreshed every night, and then divided up evenly and fairly amongst all sales reps on the list.</span>
               </div>
               <br>
            </div>
         </div>
         <h3>
            <i>Data last updated: <?php echo timeAgo($row["run_date"]);?> (updates every 24 hrs)</i>&nbsp;&nbsp;</i>
            <?php if ($issup == "yes") {	?>
            &nbsp;&nbsp;<a href="cron_job_sales_team_queue.php" target="_blank">Run Cron Job for Sales Team Queue</a>
            <?php } ?>
         </h3>
         <form method="post" name="frm_report_sales_team_list" action="report_sales_team_list.php">
            <table border="0">
               <tr>
                  <td>Employee:</td>
                  <td>
                     <select name="emp_id" id="emp_id" onchange="setseldropdown(1)">
                        ...
                        <?php
                           $qry = "SELECT employees.* FROM sales_team_queue_2 inner join employees on employees.loopID = sales_team_queue_2.assign_to_emp where assign_to_emp <> '' group by assign_to_emp order by name";
         ...
         <table width="900" border="0" cellspacing="1" cellpadding="1">
            ...
            <?php
               ...
               $tot_amount = 0; $tot_cnt1 = 0; $tot_cnt2 = 0; $tot_cnt3 = 0; $tot_cnt4 = 0; $tot_cnt5 = 0; $tot_cnt6 = 0; $tot_cnt7 = 0; $tot_cnt = 0; $tot_cnt8 = 0; $tot_cnt9 = 0;
               
               $sql = "SELECT employees.name, assign_to_emp, count(*) as cnt FROM sales_team_queue_2 inner join employees on employees.loopID = sales_team_queue_2.assign_to_emp where assign_to_emp <> '' group by assign_to_emp order by name";
               ...
               while ($myrowsel = array_shift($result)) {
               	...
               	$sql1 = "SELECT count(*) as cnt FROM sales_team_queue_2 where assign_to_emp = '" . $myrowsel["assign_to_emp"] . "' and report_criteria = 1";
         ...
         <div>
            ...
            1. Any sales accounts not contacted in > 6 months, and next step is > 15 days old. <br>
            <br><br>
         </div>
         <?php 	
            ...
            ?>		
         <?php
            $sorturl="report_sales_team_list.php?emp_id=".$_REQUEST['emp_id']."&hd_pgpost=yes"; 
            ...
            if (!isset($_REQUEST["sort"])) {
            	...
            	if ($_REQUEST["hd_pgpost"] == "yes"){
            		...
            		if (isset($_REQUEST["btntool"]) && isset($_REQUEST["emp_id"]) && $_REQUEST["emp_id"] != ""){
            
            			$sql = "SELECT * FROM sales_team_queue_2 where assign_to_emp = " . $_REQUEST['emp_id'] . " order by unqid";
            		...
            		if (isset($_REQUEST["btntool_owner"])){
            			...
            			if((isset($_REQUEST["owned_rec"])) && ($_REQUEST["owned_rec"] != ""))
            			{
            				...
            				$sql = "SELECT * FROM sales_team_queue_2 inner join companyInfo on companyInfo.ID = sales_team_queue_2.company_id where companyInfo.assignedto = " . $_REQUEST['owned_rec'] . " order by unqid";
         ...
         <table width="1000" border="0" cellspacing="1" cellpadding="1">
            ...
            <?php
               ...
               if (!isset($_REQUEST["sort"])) {
               	...
               	while ($myrowsel = array_shift($result)) {
               		...
               		$result_child = db_query("Select dateCreated, assignedto, status, last_contact_date, loopid, parent_child, parent_comp_id, industry_id from companyInfo where ID = '" . $myrowsel["company_id"] . "'");
					...
					db_b2b();
               		...
               		if ($loopid >0){
               			...
               			$result_child = db_query("Select count(*) as cnt from loop_transaction_buyer where `ignore` = 0 and warehouse_id = " . $loopid);
               
               			//echo "Select count(*) as cnt from loop_transaction_buyer where `ignore` = 0 and warehouse_id = " . $loopid . "<br>";
               			...
               			$bdescription = "View Box";
               
               			$result_child = db_query("Select bdescription from loop_boxes where id = '" . $myrowsel["inventory_loop_id"] . "'");
               
               			while ($myrowsel_child = array_shift($result_child)) {
               
               				$bdescription = $myrowsel_child["bdescription"];
               		...
               		?>
            <tr >
               <td bgcolor="#E4E4E4" ><a href="viewCompany.php?ID=<?php echo $myrowsel["company_id"]; ?>" target="_blank"><font size="2"><? echo $nickname;?></font></a></td>
               ...
               <?php
                  ...
                  if ($myrowsel["report_criteria"] == 1) { 
                  	...
                  	$condition_name = "Any sales accounts not contacted in > 6 months, and next step is > 15 days old.";
               ...
               <?php
                  ...
                  if ($parent_child == "Parent"){
                  	...
                  	$parent_child_str = "Parent Company";
            ...
            <?php
               ...
               'parent_child_str' => $parent_child_str, 'condition_name' => $condition_name, 'dateCreated_sort' => $dateCreated_sort, 'dateCreated' => $dateCreated,'assigned_name' => $assigned_name);
				...
				$MGArray = []; // Define $MGArray as an empty array
            ...
            <tr >
               ...
               <td bgcolor="#E4E4E4" ><font size="2"><?php echo $MGArraytmp2["report_criteria"]. ". " . $MGArraytmp2["condition_name"]?></font></td>
